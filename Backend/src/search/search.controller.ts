...
export default class SearchController {
  ...
  async searchInformation(
    ...
    switch (context) {
    case "citizens":
      result = await this.searchCitizen(
            criteria as string
      );
      break;
    case "messages":
      ...
      result = await this.searchMessage(
        criteria, sender, sendee
      );
      break;
    case "announcements":
      ...
      break;
    default:
      throw new BadRequestException(ErrorMessage.SEARCH_CONTEXT_UNKNOWN_MESSAGE);
    }
    return result;
  }
  ...
  async searchCitizen(username: string): Promise<ESNUser[]> {
    let users: ESNUser[] = [];
    const validStatuses = [UserStatus.GREEN, UserStatus.YELLOW, UserStatus.RED, UserStatus.UNDEFINE];
    if (validStatuses.includes(username as UserStatus)) {
      users = await this.userDao.getUsersByStatus(username);
    } else {
      users = await this.userDao.getUsersByPartialUsername(username.toLowerCase());
    }
    return users;
  }
